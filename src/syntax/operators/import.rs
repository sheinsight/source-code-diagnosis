...
impl<'a> Default for ImportVisitor<'a> {
  fn default() -> Self {
...
impl<'a> CommonTrait for ImportVisitor<'a> {
...
mod tests {
  ...
  fn get_async_function_count(usage: &Vec<CompatBox>) -> usize {
    usage
      .iter()
      .filter(|item| item.name == "operators_import")
      .count()
  }
  ...
  fn should_ok_when_async_generator_function_declaration() {
    ...
    let usage = tester.analyze("import('./modules/myModule.js')");
