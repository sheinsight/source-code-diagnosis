...
pub struct SetComputedPropertyNamesVisitor<'a> {
  usage: Vec<CompatBox>,
  parent_stack: Vec<AstKind<'a>>,
  compat: Compat,
}

impl<'a> Default for SetComputedPropertyNamesVisitor<'a> {
  fn default() -> Self {
...
impl<'a> CommonTrait for SetComputedPropertyNamesVisitor<'a> {
  fn get_usage(&self) -> Vec<CompatBox> {
    self.usage.clone()
  }
}

impl<'a> Visit<'a> for SetComputedPropertyNamesVisitor<'a> {
...
mod tests {
  ...
  fn get_async_function_count(usage: &Vec<CompatBox>) -> usize {
    usage
      .iter()
      .filter(|item| item.name == "set_computed_property_names")
      .count()
  }
  ...
  fn should_ok_when_async_generator_function_declaration() {
    let mut tester =
      SemanticTester::from_visitor(SetComputedPropertyNamesVisitor::default());
