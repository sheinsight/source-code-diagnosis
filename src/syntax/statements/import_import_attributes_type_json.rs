...
pub struct ImportImportAttributesTypeJsonVisitor {
  usage: Vec<CompatBox>,
  compat: Compat,
}

impl Default for ImportImportAttributesTypeJsonVisitor {
  fn default() -> Self {
    let usage: Vec<CompatBox> = Vec::new();
    let compat: Compat =
      from_str(include_str!("./import_import_attributes_type_json.json"))
        .unwrap();
    Self { usage, compat }
  }
}

impl CommonTrait for ImportImportAttributesTypeJsonVisitor {
  fn get_usage(&self) -> Vec<CompatBox> {
    self.usage.clone()
  }
}

impl<'a> Visit<'a> for ImportImportAttributesTypeJsonVisitor {
...
mod tests {
  ...
  fn get_async_function_count(usage: &Vec<CompatBox>) -> usize {
    usage
      .iter()
      .filter(|item| item.name == "import_import_attributes_type_json")
      .count()
  }
  ...
  fn should_ok_when_import_attributes_type_json() {
    let mut tester = SemanticTester::from_visitor(
      ImportImportAttributesTypeJsonVisitor::default(),
    );
    let usage =
  ...
  #[test]
  fn should_fail_when_import_attributes_type_css() {
    let mut tester = SemanticTester::from_visitor(
      ImportImportAttributesTypeJsonVisitor::default(),
    );
    let usage =
